...
<h2 style="font-size: 1.15rem;">Example 1: <code>selectInterChallenge()</code></h2>
...
<pre><code>function selectInterChallenge(subject) {
    ...
    // Store which subject the student clicked and reset hint usage for the new challenge
    currentInterChallenge = subject;
    hintsUsed = 0;

    // Reset every subject button back to its default style before highlighting the new one
    ...
    // Find the button for the selected subject and apply the active highlight styling
    ...
    // Clear the old student response and hide any hint, AI response, or example answer from the last round
    ...
    // Show the challenge section so the student can start working on the new prompt
    document.getElementById('inter-challenge').classList.remove('hidden');

    // Look up the weak prompt for the chosen subject and display it on the page
    ...
    // Reset analysis UI like the score bar, checkmarks, and feedback messages
    resetAnalysis();
}</code></pre>
...
<pre><code>// This helper is only responsible for updating challenge state when a new subject is chosen
function setCurrentChallenge(subject) {
    currentInterChallenge = subject;
    hintsUsed = 0;
}

// This helper only updates button styling so the selected subject looks active
function highlightSubjectButton(subject) {
    ...
    // After resetting every button, highlight the one the student actually clicked
...
// This helper only clears old input and hides leftover panels from the previous attempt
...
// This helper only reads the weak prompt for the chosen subject and places it on the page
function displayWeakPrompt(subject) {
    document.getElementById('weak-prompt-display').textContent = interChallenges[subject].weak;
}

// This main function now acts like a coordinator by calling each small helper in order
function selectInterChallenge(subject) {
    ...
    // Reset the analysis UI for the new challenge and make the challenge section visible
    resetAnalysis();
    document.getElementById('inter-challenge').classList.remove('hidden');
}</code></pre>
...
<ul>
    <li>Each new function has exactly one job. <code>highlightSubjectButton()</code> only touches button colors. <code>clearChallengeInputs()</code> only resets the form. They don't know about each other and don't interfere.</li>
    <li>Easier to fix bugs. If button highlighting breaks, you go straight to <code>highlightSubjectButton()</code>. You don't have to scroll through 30 lines of unrelated code to find the problem.</li>
    <li>Functions are now reusable. <code>clearChallengeInputs()</code> can be called from anywhere else in the project that needs to reset the form. Before, that logic was trapped and couldn't be reused.</li>
    <li>The coordinator function stays clean. <code>selectInterChallenge()</code> still exists but now it just calls the smaller functions in order. You can read it like a checklist and immediately understand what happens when a student picks a subject.</li>
</ul>

<hr>

<h2 style="font-size: 1.15rem;">Example 2: <code>analyzeStudentPrompt()</code></h2>
...
<h3>How this follows SRP:</h3>
<ul>
    <li><code>scorePrompt()</code> is pure logic with no screen updates. It takes in text and a subject name, checks keywords, and just returns a score and a set of true/false values. Because it doesn't touch the screen at all, you could test it by just calling it with a string — no browser or HTML needed.</li>
    <li><code>renderScoreBar()</code> only cares about the bar. If the design changes and you want different colors or a different style, this is the only function you need to touch. Nothing else is affected.</li>
    <li><code>renderFeedback()</code> only handles messages. If you want to reword any of the feedback text, it's all in one place. You don't have to search through scoring logic to find it.</li>
    <li>The coordinator just connects everything. <code>analyzeStudentPrompt()</code> now just reads the input, calls <code>scorePrompt()</code>, and passes the result to the two render functions. It doesn't do any of that work itself — it just coordinates.</li>
    <li>The biggest win here is that the scoring logic and the screen-drawing code are now completely separate. Before, they were the same thing. Mixing logic and visuals in the same function is one of the most common SRP violations, and this fixes it cleanly.</li>
</ul>
